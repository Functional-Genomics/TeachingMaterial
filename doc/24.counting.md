...
wget ftp://ftp.ensembl.org/pub/release-82/gtf/drosophila_melanogaster/\
Drosophila_melanogaster.BDGP6.82.gtf.gz

gunzip -d Drosophila_melanogaster.BDGP6.82.gtf.gz
```

Which information does it contain? [Hint](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex1.md)
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex2.md)
...
samtools view untreated3_paired_byname.bam | htseq-count \
    --mode=intersection-nonempty \
    --stranded=no \
    --type=exon \
    --idattr=gene_id - \
    ../../reference/drosophila_melanogaster/Drosophila_melanogaster.BDGP6.82.gtf > htseq_count.out
```
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex4.md)
...
names(which)="4" #we are focusing on the chromosome 4
which
param=ScanBamParam(which=which)

# import the data
aln_chr4=readGAlignmentPairs("mapped/untreated3_paired.bam", use.names=T)
aln_chr4
```
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex5.md)

Since we have loaded only the reads that map to chromosome 4, we can proceed to modify the `aln_chr4` object accordingly:

```rconsole
seqlevels(aln_chr4)="4"
aln_chr4
```
...
ensembl82=useMart(host="sep2015.archive.ensembl.org", 
...
annot=getBM( fields, mart=ensembl82)
```

##### Exercise 6:
Have a look at the newly created `annot` object. What type of object is it?
*Hint:* use the function *class*.
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex6.md)
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex7.md)
...
Now that we have the alignment locations (`aln_chr4` object) and the genome annotation (`annot` object), we can quantify gene expression by counting reads over all exons of a gene and summing them together. Similarly to what we encountered with htseq-count, we need to pay attention to those reads that overlap with several features.
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex8.md)
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/EMBLPredocCourse2015/solutions/_counting_ex9.md)
