...
#### Exercise 1: 
0ne of the input files required by htseq-count is a GTF file. For this practical, you will find this file under the directory `reference`. 

```bash
#DO NOT RUN - ALREADY PROVIDED in the reference folder

# This file has been already retrieved from the ensembl archive:
wget ftp://ftp.ensembl.org/pub/release-76/gtf/homo_sapiens/\
Homo_sapiens.GRCh38.76.gtf.gz

gunzip Homo_sapiens.GRCh38.76.gtf.gz
```

Which information does it contain? [Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex1.md)


#### Exercise 2: 
As we have already mentioned, the other required input file is a BAM file. Can you spot any specific requirement regarding this file?
[*Hint*](http://www-huber.embl.de/users/anders/HTSeq/doc/count.html) - 
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex2.md)
...
#### Exercise 3: 
What are the differences between these three counting modes?
[*Hint*](http://www-huber.embl.de/users/anders/HTSeq/doc/count.html)
...
samtools view ERR315494_paired.bam | htseq-count \
    --mode=intersection-nonempty \
    --stranded=no \
    --type=exon \
    --idattr=gene_id - \
    ../../reference/Homo_sapiens.GRCh38.76.gtf > htseq_count.out
```

#### Exercise 4:
In addition to the counts that overlap known genes, the output file also contains some extra information on reads that could not be assigned to any of those; can you find it?
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex4.md)
...
# import the data
aln_chr21=readGAlignmentPairs("../data/mapped/ERR315494.bam", use.names=T, param=param)
aln_chr21
```
...
##### Exercise 5:
After having a look at the *GenomicRanges* and *GenomicAlignments* documentations, try to answer the following questions:
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex5.md)
...
ensembl76=useMart(host="aug2014.archive.ensembl.org", 
...
annot=getBM( fields, mart=ensembl76)
```

##### Exercise 6:
Have a look at the newly created `annot` object. What type of object is it?
*Hint:* use the function *class*.
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex6.md)

##### Exercise 7:
In the next subsection we will calculate the overlap between the loaded BAM file and the annotation with the function *summarizeOverlaps* from the *GenomicRanges* package. What is the input required? Do we have all the necessary objects ready?
*Hint:* type `?summarizeOverlaps`.
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex7.md)
...
#### Exercise 10: 
How many elements does the vector `exon_counts` contain? Why is that? *Hint:* use the function *length*.
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_counting_ex8.md)

Let us subset the counts for chromosome 21:

```rconsole
genes_chr21=unique(elementMetadata(annot[seqnames(annot)=="chr21"])$gene)
exon_counts_chr4=exon_counts[names(exon_counts) %in% genes_chr21]
```

#### Exercise 9:
So far we have obtained the number of reads overlapping each exon. How can we combine this information to obtain gene counts?
*Hint:* use the functions *split* and *sapply*.
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions//_counting_ex9.md)
