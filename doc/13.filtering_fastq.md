...
```bash
zcat ERR315414_1.fastq.gz | prinseq-lite \
    -fastq stdin \
    -out_good ERR315414_1_filt1 \
    -out_bad null \
    -trim_qual_right 30 -min_len 32 \
    -ns_max_p 5 \
    -lc_method dust -lc_threshold 10
    
zcat ERR315414_2.fastq.gz | prinseq-lite \
    -fastq stdin \
    -out_good ERR315414_2_filt1 \
    -out_bad null \
    -trim_qual_right 30 -min_len 32 \
    -ns_max_p 5 \
    -lc_method dust -lc_threshold 10
...
[Solution](https://github.com/Functional-Genomics/TeachingMaterial/blob/Cancer-Genomics-07-2015/solutions/_filtering_fastq_ex1.md)
...
### Exercise 3: 
After filtering the fastq file it is not a bad idea to obtain a QA report again to decide whether we are happy with the results. Do you think we got rid of the main issues spotted initially?
...
```bash
fastq-mcf adapters.fa ERR315414_1.fastq.gz ERR315414_2.fastq.gz \
    -o ERR315414_1_filt2.fastq -o ERR315414_2_filt2.fastq \
